...
export function useAutoUpdateLayoutState<T>(
...
export function useAutoUpdateLayoutState<T>(
  state: Exclude<T, (...args: any[]) => any>,
  callback: (state: T) => T
): [T, (value: SetStateAction<T>, skipCallback?: boolean) => T];
// state 와 callback 함수를 받는 경우 (state 에 function 지정 불가)
export function useAutoUpdateLayoutState<T = never, StateT = never>(
...
// 구현부
export function useAutoUpdateLayoutState(state: any, callback?: any) {
